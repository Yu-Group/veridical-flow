...
class ModuleSet:
    def __init__(self, name: str = '', modules: dict = {}):
        ...
        Params
        -------
        modules is a dictionary of functions that we want to associate with 
        '''
        self.name = name 
    ...
    def apply_func(self,*args,matching = 'cartesian',order = 'typical',**kwargs):
        ...
        *args: Two types currently allowed: 
        ...
        Returns
        -------
        results: dict with items being determined by functions in module set. Functions and input dictionaries are currently             matched using  matching = 'cartesian' format. e.g. module = {LR : logistic}, data = {train_1 : [X1,y1], train2 : [X2,y2]}         then output will be output_dict = {(train_1,LR)  : fitted logsistic, (train_2,LR) :  fitted logistic}. Currently matching          = 'subset is not used...
        '''
        dict_present = False
        for ele in args:
